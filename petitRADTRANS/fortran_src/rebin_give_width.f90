...
subroutine rebin_give_width(nu_synth, flux_synth, nu_obs, bin_width, reb_synth_flux, len_obs, len_synth)
    implicit none
    ! I/O
    ...
    do i_nu = 1, len_obs
        ...
        do while (nu_synth(i_nu_synth) < nu_obs(i_nu) + bin_width(i_nu) / 2d0)
            ...
            add_fl = (y_ax - slope * nu_synth(i_nu_synth - 1)) * (nu2 - nu1) + slope * (nu2 - nu1) * (nu2 + nu1) / 2d0
