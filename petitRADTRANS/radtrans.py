...
class Radtrans(_read_opacities.ReadOpacities):
            ...
            for c in continuum_opacities:
                ...
                if not c == 'H-':
                    ...
                    if  os.path.isdir(self.path+'/opacities/continuum/CIA/'+c) is False: #correct
                        ...
                        cia_temp_dims,cia_lambda_dims = \
                          fi.cia_read(c,self.path)
                        cia_alpha_grid = np.array(cia_alpha_grid, \
                                                              dtype='d',order='F')
                        cia_temp = cia_temp[:cia_temp_dims]
                        cia_lambda = cia_lambda[:cia_lambda_dims]
                        cia_alpha_grid = \
                                  cia_alpha_grid[:cia_lambda_dims,:cia_temp_dims]
                        species = {
                            'id': c,
                            'molecules' : mol,
                            'weight' : weight,
                        ...
                        #cia=pd.DataFrame(data=species['alpha'],index=species['lambda'],columns=species['temperature'])
                        #plots(cia,str(c)+'.pdf')
                        #print(species['lambda'].min(),species['lambda'].max())
    ...
    def interpolate_cia(self,key,mfrac):
          ...
          if x.shape[0]>1:
              ...
              f=interpolate.interp1d(x,z,kind='linear',bounds_error=False,fill_value=(z[:,0],z[:,-1]),axis=1)
              z_temp2=f(xnew)
              f1=interpolate.interp1d(y,z_temp2,kind='linear',bounds_error=False,fill_value=(-16),axis=0)

              znew=10**f1(ynew)
              znew= np.where(znew<1.00001e-16,0,znew)
              #import pylab as plt
              #plt.clf()
              #plt.loglog(ynew,znew)
              #plt.savefig('cia_local_extrap.png')

              return np.multiply(znew,factor)
          else:
              print('ERROR! pRT needs a rectangular CIA table.')
    ...
    def mix_opa_tot(self, abundances, mmw, gravity, \
        ...
        self.mmw = mmw
        for i_spec in range(len(self.line_species)):
            #update: after resolution string addition
            self.line_abundances[:,i_spec] = \
                abundances[self.line_species[i_spec].split('_')[0]]
        ...
        for key in self.CIA_species.keys():
            ...
            for m in self.CIA_species[key]['molecules']:
                ...
                abund= abund*np.mean(abundances[m])

        if self.N2N2CIA:
            self.continuum_opa = self.continuum_opa + \
                self.interpolate_cia(key,np.sqrt(abund))
                #self.CIA_species[key]['lambda'],\
                #    self.CIA_species[key]['temperature'], self.CIA_species[key]['alpha'],\
                #    np.sqrt(abund),np.sqrt(self.CIA_species[key]['weight']))
        #import pylab as plt
        #plt.loglog(nc.c/self.freq/1e-4,self.continuum_opa)
        #plt.savefig('local_CO2.png')
        # Calc. H- opacity
        if self.Hminus:
            self.continuum_opa = \
